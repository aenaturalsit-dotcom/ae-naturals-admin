...
export default function AdminOrdersPage() {
  ...
  const ordersData = useMemo(() => {
    if (!data) {
      return {
        orders: [],
        meta: { total: 0, page: Number(page), limit: 15, totalPages: 1 },
      };
    }
    ...
    if (data && Array.isArray((data as any).data)) {
      const shapedData = data as {
        data: Order[];
        meta?: ListOrdersResponse["meta"];
      };
      return {
        orders: shapedData.data,
        meta: shapedData.meta || {
          total: shapedData.data.length,
          page: Number(page),
          limit: 15,
          totalPages: Math.ceil(shapedData.data.length / 15),
        },
      };
    }
    ...
    if (data && data.data && Array.isArray((data.data as any).data)) {
      ...
      return {
        ...
        meta: shapedData.data.meta || {
          total: shapedData.data.data.length,
          page: Number(page),
          limit: 15,
          totalPages: Math.ceil(shapedData.data.data.length / 15),
        },
      };
    }
  ...
  const bulkUpdateMutation = useMutation({
    mutationFn: () =>
      AdminOrdersService.bulkUpdateStatus(selectedRowIds, bulkStatus),
    onSuccess: () => {
      ...
      {selectedRowIds.length > 0 && (
        <div className="mb-4 p-4 bg-[#217A6E]/10 border border-[#217A6E]/20 rounded-xl flex flex-wrap items-center justify-between gap-3 animate-in slide-in-from-top-2">
          <span className="font-semibold text-[#217A6E]">
            ...
            {selectedRowIds.length === 1 ? "order" : "orders"} selected
          </span>
