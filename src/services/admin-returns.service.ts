...
export const AdminReturnsService = {
  ...
  approveReturn: async (
    id: string,
    data: { adminComment?: string; pickupScheduledAt?: string },
  ) => {
  ...
  rejectReturn: async (
    id: string,
    data: { rejectionReason: string; adminComment?: string },
  ) => {
  ...
  schedulePickup: async (id: string, data: { pickupDate: string }) => {
    const response = await apiClient.patch(
      `/admin/returns/${id}/schedule-pickup`,
      data,
    );
    return response.data || response;
  },
  ...
  completePickup: async (id: string) => {
    const response = await apiClient.patch(
      `/admin/returns/${id}/complete-pickup`,
    );
    return response.data || response;
  },
  ...
  receiveReturn: async (
    id: string,
    data: { items?: any[]; adminComment?: string },
  ) => {
    const response = await apiClient.patch(
